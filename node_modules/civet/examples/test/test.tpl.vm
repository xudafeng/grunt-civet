    <link rel="stylesheet" href="try-list.css"/>
    <div class="br_try_on_s">
        <h4 class="btos_header"></h4>
        #foreach($!card in $!lists)
#set($!index=$!velocityCount)
        <div class="br_try_on_card br_inlines">
            <div class="btoc_left br_inline">
                <img src="$!{card.picUrl}" width="360" height="360"/>
                #if($!card.type == 'free')<div class="btocl_icon "></div>#end
            </div>
            <div class="btoc_right br_inline">
                <div class="btocr_up">
                    ...
                    #if($!index == 2)
                    下标测试$!{card.type}
                        #foreach($!card2 in $!list2)
#set($!index2=$!velocityCount)
                            下标嵌套作用域测试 $!{index2}
                        #end
                    #end
                    #foreach($!card3 in $!list3)
                    无下标干扰测试
                    #end
                    #if($!card.type == 'fuyou')
                ...
                <div class="btocr_middle">
                    ...
                    <div>
                        <span class="btocr_date_text">
                            12<span class="btocr_font_14">月</span>22<span class="btocr_font_14">日</span><span class="btocr_font_16"> 00:00</span>
                        </span>
                    </div>
                    ...
                    #else
                    #if($!card.status == 1)
                    <div>已申请：<span class="btoc_number btocr_font_bigest btocr_font_red">10000</span>人</div>
                    <a class="btocr_button">付邮试用</a>
                    #elseif($!card . status == 2)
                    <div><span class="btocr_font_gray">试用已结束</span></div>
                    <a class="btocr_link_to_report" href="#">查看报告(123)>></a>
                    #else
                    <div>
                        <span class="btocr_date_text">
                            12<span class="btocr_font_14">月</span>22<span class="btocr_font_14">日</span><span class="btocr_font_16"> 00:00</span>
                ...
                <div class="btocr_down">
                    ...
                    <p>1.所有会员都可以免费申请，申请成功无需支付邮费</p>
                    <p>2.申请成功需要提交真是原创的试用报告</p>
                </div>
                #if($!card.status == 2)
